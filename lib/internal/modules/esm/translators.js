...
function initTSParse() {
  tsParse ??= require('internal/deps/swc/wasm').transformSync;
  return tsParse;
}

// Strategy for loading a TypeScript module
translators.set('typescript', async function(url, source) {
  assertBufferSource(source, false, 'load');
  const parse = initTSParse();
  const { code } = parse(stringify(source), {
    swcrc: false,
    jsc: {
      parser: {
        syntax: 'typescript',
  ...
  const { compileSourceTextModule } = require('internal/modules/esm/utils');
  const module = compileSourceTextModule(url, code, this);
  return module;
});
