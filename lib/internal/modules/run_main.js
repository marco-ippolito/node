...
function shouldUseTSLoader(mainPath) {
  // Determine the module format of the entry point.
  if (mainPath && StringPrototypeEndsWith(mainPath, '.ts')) { return true; }
}
...
function executeUserEntryPoint(main = process.argv[1]) {
  ...
  const tsEnabled = getOptionValue('--experimental-typescript');
  const useTSLoader = tsEnabled && shouldUseTSLoader(resolvedMain);
  // If file is ts we should consider it as esm
  const useESMLoader = useTSLoader || shouldUseESMLoader(resolvedMain);
  let mainURL;
  // Unless we know we should use the ESM loader to handle the entry point per the checks in `shouldUseESMLoader`, first
  // try to run the entry point via the CommonJS loader; and if that fails under certain conditions, retry as ESM.
  if (useTSLoader) {
    register('node:internal/modules/ts/typescript', {
      __proto__: null,
    });
  }
