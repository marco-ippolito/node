...
void GetTranspiledCachedModule(const FunctionCallbackInfo<Value>& args) {
  CHECK(args[0]->IsString());  // Source
  CHECK(args[1]->IsString());  // Filename
  ...
  cache_entry = env->compile_cache_handler()->Get(
      code, filename, CachedCodeType::kTypeScript);

  if (cache_entry != nullptr) {
    Local<Value> values[2] = {
        String::NewFromUtf8(isolate,
                            cache_entry->cache_filename.data(),
                            NewStringType::kNormal,
                            cache_entry->cache_filename.size())
            .ToLocalChecked(),
        String::NewFromUtf8(isolate,
                            cache_entry->source_filename.data(),
                            NewStringType::kNormal,
                            cache_entry->source_filename.size())
            .ToLocalChecked(),
    };
    args.GetReturnValue().Set(Array::New(isolate, values, 2));
  }
}

void SetTranspiledCachedModule(const FunctionCallbackInfo<Value>& args) {
  CHECK(args[0]->IsString());  // Source
  CHECK(args[1]->IsString());  // Filename
  ...
  cache_entry = env->compile_cache_handler()->Set(
      code, filename, CachedCodeType::kTypeScript);
...
void BindingData::CreatePerIsolateProperties(IsolateData* isolate_data,
  ...
  SetMethod(
      isolate, target, "getTranspiledCachedModule", GetTranspiledCachedModule);
  SetMethod(
      isolate, target, "setTranspiledCachedModule", SetTranspiledCachedModule);
}
